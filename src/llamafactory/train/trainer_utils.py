...
def get_student_device_from_env():
    local_rank = int(os.getenv("LOCAL_RANK", "0"))
    return torch.device(f"cuda:{local_rank}")

def get_teacher_device_from_env():
    explicit = os.getenv("TEACHER_DEVICE")
    if explicit is not None:
        return torch.device(f"cuda:{int(explicit)}")

    ndev = torch.cuda.device_count()
    local_rank = int(os.getenv("LOCAL_RANK", "0"))
    stride = int(os.getenv("TEACHER_STRIDE", "4")) 
    if ndev == 0:
        raise RuntimeError("No CUDA device visible.")
    if ndev == 1:
        return torch.device("cuda:0")
    return torch.device(f"cuda:{(local_rank + stride) % ndev}")
