...
class Qwen2VLPlugin(BasePlugin):
    ...
    def _regularize_videos(
        ...
        for video in videos:
            with av.open(video, mode="r", options={"threads": max_threads}) as container:
                video_stream = next(s for s in container.streams if s.type == "video")

                try:
                    cc = video_stream.codec_context
                    cc.thread_type = "FRAME"     
                    cc.thread_count = int(max_threads)
                except Exception:
                    pass  

                sample_indices = self._get_video_sample_indices(video_stream, **kwargs)
                sample_set = set(sample_indices)
                max_idx = max(sample_set) if sample_set else -1
                ...
                fps = None
                try:
                    if getattr(video_stream, "average_rate", None):
                ...
                if fps is None:
                    if video_stream.duration is None or video_stream.time_base is None:
                        ...
                        fps_est = (len(sample_indices) / duration_s) if duration_s > 0 else 2.0
