...
class Cache(transformers.cache_utils.Cache):
    ...
    def detach(self) -> "Cache":
        """
        返回一个新的 Cache，其中所有 Tensor 都被 detach()，
        用于 TBTT 分块时截断反传图。
        """
        new_cache = Cache(seen_tokens=self._seen_tokens)

        for state in self.states:
            new_state = {}
            if state.get("recurrent_state") is not None:
                new_state["recurrent_state"] = state["recurrent_state"].detach()

            if state.get("attn_state") is not None:
                k, v = state["attn_state"]
                new_state["attn_state"] = (k.detach(), v.detach())

            if state.get("conv_state") is not None:
                new_state["conv_state"] = tuple(t.detach() for t in state["conv_state"])

            if state.get("ffn_state") is not None:
                new_state["ffn_state"] = state["ffn_state"].detach()

            new_cache.states.append(new_state)
