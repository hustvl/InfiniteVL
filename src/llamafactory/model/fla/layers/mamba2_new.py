...
class Mamba2_new(nn.Module):
    def __init__(self,
        ...
        self.v_proj = nn.Linear(self.hidden_size, self.num_heads * self.head_dim*2, bias=True)
        ...
        self.k_shape = [self.num_heads, self.head_dim]
        self.v_shape = [self.num_heads, self.head_dim*2]
        
        if self.use_short_conv:
            conv_dim = self.num_heads * self.head_dim + self.num_heads * self.head_dim + 2 * self.num_heads * self.head_dim
        ...
        if self.use_gnorm:
            self.g_norm = RMSNorm(hidden_size=self.head_dim * 2, elementwise_affine=elementwise_affine, eps=norm_eps).to(self.dtype).to(self.device)
    ...
    def forward(self,
        ...
        v = rearrange(v, "b h l n -> b l h n", h=self.num_heads)
        q = rearrange(q, "b h l n -> b l h n", h=self.num_heads)
        k = rearrange(k, "b h l n -> b l h n", h=self.num_heads)
        ...
        if self.use_short_conv:
            ...
            v = rearrange(v, "b l (h n) -> b l h n", h=self.num_heads)
            k = rearrange(k, "b l (h n) -> b l h n", h=self.num_heads)
            q = rearrange(q, "b l (h n) -> b l h n", h=self.num_heads)

        else:
            q, k ,v = self.act(q), self.act(k), self.act(v)
            
        dt = self.in_proj(hidden_states)
        if self.use_A:
